...
## 2024-05-28 - Support WORLD_MENU intention in custom UI text inputs
**Learning:** In custom UI text inputs like `InputMenu`, checking only for the hardcoded `buttons.START` to confirm input breaks navigation for users who rely on custom keybindings or alternative input methods mapped to the generic `WORLD_MENU` intention.
**Action:** When implementing input confirmation or menu actions triggered by 'START', always check for `intentions.WORLD_MENU` alongside `buttons.START` to maintain accessibility.
