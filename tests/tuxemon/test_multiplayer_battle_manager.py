...
def test_load_log_supports_legacy_key_names() -> None:
    challenge_id = uuid4()
    challenger = uuid4()
    challenged = uuid4()
    manager = MultiplayerBattleManager()
    ...
    manager.load_log(
        {
            "pending_challenges": [
                {
                    "proposing_player_id": str(challenger),
                    "receiving_player_id": str(challenged),
                    "offer_id": str(challenge_id),
    ...
    assert manager.pending_challenges[0].challenge_id == challenge_id


def test_load_log_tolerates_malformed_entries_and_bad_ttl() -> None:
    manager = MultiplayerBattleManager()
    manager.default_challenge_ttl_seconds = 180
    challenger = uuid4()
    challenged = uuid4()
    valid = {
    ...
    manager.load_log(
        {
            ...
            "default_challenge_ttl_seconds": "nonnumeric",
        }
    )
